...
export default function ValidacionCorreccionesPage() {
  ...
  const seleccionIds = useMemo(
    () => searchParams.get("seleccion")?.split(",") ?? [],
    [searchParams]
  );
  ...
  useEffect(() => {
    const fetchRequisitos = async () => {
      try {
        const data = await requisitoService.obtenerRequisitosConCorrecciones(
          proyectoId,
          seleccionIds
        );
        setRequisitos(data);
    ...
    <DashboardLayout>
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <p className="text-gray-700 mb-6 text-lg">
          A continuación se presentan los requisitos junto con las correcciones seleccionadas.
          Verifique y valide las modificaciones propuestas.
        </p>
